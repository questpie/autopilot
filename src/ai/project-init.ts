...
1. \`autopilot.config.ts\` — A valid TypeScript config file that exports a ProjectConfig object.
   Required shape:
   \`\`\`typescript
...
2. \`handoff.md\` — A summary of:
   - what was analyzed
   - what tasks/epics were created
   - key decisions made
...
- Default provider: ${opts.projectName.includes("codex") ? "codex" : "claude"}
- Default mode: autonomous
- Default permission profile: elevated
`);
...
export async function initProject(opts: InitOptions): Promise<{
  ...
  const configExists = existsSync(`${outputDir}/autopilot.config.ts`);
  if (!configExists) {
