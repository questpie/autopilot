...
import { InputGroup, InputGroupAddon } from "@questpie/ui/components/ui/input-group";
		...
		<div data-slot="command-input-wrapper" className="border-b border-hairline p-3 md:p-4">
			<InputGroup className="h-[var(--touch-target)]! overflow-hidden rounded-none! border-0 bg-transparent shadow-none! md:h-12!">
				...
				<CommandPrimitive.Input
					...
					className={cn(
						"h-full w-0! min-w-0 flex-1 box-border text-base font-medium tracking-tight outline-hidden disabled:cursor-not-allowed disabled:opacity-50 md:text-xl",
						className,
					)}
					{...props}
				/>
...
function CommandItem({
	...
	return (
		<CommandPrimitive.Item
			...
			className={cn(
				"group/command-item relative flex min-h-9 cursor-default items-center gap-2 rounded-[var(--radius-sm)] px-2 py-1.5 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-selected:bg-muted data-selected:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-selected:*:[svg]:text-foreground",
				className,
			)}
			{...props}
		>
