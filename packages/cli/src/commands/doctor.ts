...
const RUNTIME_BINARIES: Record<string, string | null> = {
	'claude-code': 'claude',
	codex: null,
	opencode: 'opencode',
}
	...
	try {
		...
		checks.push(
			warn(
				'cli-version',
				'CLI version',
				'Could not read CLI package version.',
				'Reinstall @questpie/autopilot or check package.json integrity.',
			),
		)
	}
}

async function resolveCompanyRoot(
	companyRoot: string | undefined,
	cwd: string,
...
function checkCompanyRoot(ctx: DoctorContext, checks: DoctorCheck[]): void {
	...
	checks.push(
		hasYamlFiles(agentsDir)
			? pass('agents', 'Agents config', `Found agent definitions in ${PATHS.AGENTS_DIR}.`)
			: warn(
					'agents',
					'Agents config',
					`No agent YAML files found in ${PATHS.AGENTS_DIR}.`,
					'Run `autopilot bootstrap --yes` or add an agent definition.',
				),
	)
	checks.push(
		hasYamlFiles(workflowsDir)
			? pass(
					'workflows',
					'Workflow config',
					`Found workflow definitions in ${PATHS.WORKFLOWS_DIR}.`,
				)
			: warn(
					'workflows',
					'Workflow config',
					`No workflow YAML files found in ${PATHS.WORKFLOWS_DIR}.`,
					'Run `autopilot bootstrap --yes` or add a workflow definition.',
				),
	)
}
	...
	if (!masterKey) {
		checks.push(
			production
				? fail(
						...
						'AUTOPILOT_MASTER_KEY is missing in production.',
						'Generate one with `openssl rand -hex 32`.',
					)
				: warn(
						'master-key',
						'Shared secret encryption key',
						'AUTOPILOT_MASTER_KEY is not set; shared secret operations will fail until configured.',
						'Generate one with `openssl rand -hex 32`.',
					),
		)
		...
		checks.push(
			fail(
				'master-key',
				'Shared secret encryption key',
				'AUTOPILOT_MASTER_KEY must be exactly 64 hex characters.',
				'Generate one with `openssl rand -hex 32`.',
			),
		)
	} else {
		checks.push(
			pass(
				'master-key',
				'Shared secret encryption key',
				'AUTOPILOT_MASTER_KEY has the expected 64-hex format.',
			),
		)
	}
		...
		checks.push(
			production
				? fail(
						'auth-secret',
						'Better Auth secret',
						'BETTER_AUTH_SECRET is missing in production.',
						'Generate one with `openssl rand -hex 32`.',
					)
				: warn(
						'auth-secret',
						'Better Auth secret',
						'BETTER_AUTH_SECRET is not set; production auth will fail until configured.',
						'Generate one with `openssl rand -hex 32`.',
					),
		)
...
function checkUrlEnv(ctx: DoctorContext, checks: DoctorCheck[]): void {
	...
	if (orchestratorUrl) {
		checks.push(
			isValidUrl(orchestratorUrl)
				? pass('orchestrator-url', 'Orchestrator URL', `ORCHESTRATOR_URL is ${orchestratorUrl}.`)
				: fail(
						'orchestrator-url',
						'Orchestrator URL',
						'ORCHESTRATOR_URL is not a valid URL.',
						'Use a full URL such as `https://autopilot.example.com` or `http://SERVER_IP:7778`.',
					),
		)
	} else {
		checks.push(
			warn(
				'orchestrator-url',
				'Orchestrator URL',
				'ORCHESTRATOR_URL is not set; rendered links may fall back to localhost.',
				'Set ORCHESTRATOR_URL for VPS, reverse-proxy, LAN, or Tailscale deployments.',
			),
		)
	}
	...
	if (!corsOrigin) {
		checks.push(
			warn(
				'cors-origin',
				'CORS origin',
				'CORS_ORIGIN is not set; the API will fall back to the orchestrator origin.',
				'Set CORS_ORIGIN when operators use a separate browser/API origin.',
			),
		)
		return
	}
	...
	checks.push(
		invalid.length === 0
			...
			: fail(
					'cors-origin',
					'CORS origin',
					`CORS_ORIGIN contains invalid URL values: ${invalid.join(', ')}.`,
					'Use comma-separated full URLs.',
				),
	)
}
	...
	if (!existsSync(dockerCompose) && !existsSync(deployCompose) && !existsSync(dockerfile)) {
		checks.push(
			warn(
				...
				'No Docker packaging files found in the current directory.',
				'Run this check from the Autopilot repo root to validate Docker packaging.',
			),
		)
		return
	}
	...
	checks.push(
		existsSync(dockerfile)
			...
			: warn(
					...
					'Use repo Docker packaging or documented deploy files.',
				),
	)
	checks.push(
		existsSync(entrypoint)
			...
			: warn(
					'docker-entrypoint',
					'Docker entrypoint',
					'docker-entrypoint.sh is missing.',
					'Use the repo entrypoint for orchestrator-only Docker boot.',
				),
	)

	for (const [id, file] of [
		['compose-root', dockerCompose],
		['compose-deploy', deployCompose],
	] as const) {
		...
		checks.push(
			content.includes('/api/health')
				? pass(
						id,
						file.endsWith('deploy/docker-compose.yml')
							? 'Deploy compose healthcheck'
							: 'Root compose healthcheck',
						`${file} uses /api/health.`,
					)
				: fail(
						id,
						file.endsWith('deploy/docker-compose.yml')
							? 'Deploy compose healthcheck'
							: 'Root compose healthcheck',
						`${file} does not reference /api/health.`,
						'Use /api/health for Docker healthchecks.',
					),
		)
	}
}

function checkRuntimeBinaries(
	runtimes: string[],
	checks: DoctorCheck[],
	requireRuntime: boolean,
): void {
	if (runtimes.length === 0) {
		checks.push(
			warn('runtime-any', 'Worker runtime availability', 'Runtime binary checks were skipped.'),
		)
		return
	}
	...
	for (const runtime of runtimes) {
		const configuredBinary = RUNTIME_BINARIES[runtime]
		const binary = configuredBinary === undefined ? runtime : configuredBinary
		if (binary === null) {
			foundAny = true
			checks.push(pass(`runtime-${runtime}`, `Runtime: ${runtime}`, 'Provided by spawn-agent.'))
			continue
		}
	...
	if (!foundAny) {
		...
		checks.push(
			check(
				...
				'No supported runtime binary was found in PATH.',
				'Install and authenticate at least one runtime on worker machines, or omit --require-runtime on orchestrator-only hosts.',
			),
		)
	} else {
		checks.push(
			pass(
				'runtime-any',
				'Worker runtime availability',
				'At least one supported runtime binary is available.',
			),
		)
	}
}
...
async function checkOrchestratorHealth(
	...
	if (options.offline) {
		checks.push(
			warn('orchestrator-health', 'Orchestrator health', 'Skipped because --offline was set.'),
		)
		return
	}
	...
	if (!url) {
		checks.push(
			warn(
				'orchestrator-health',
				'Orchestrator health',
				'No orchestrator URL configured for health check.',
				'Pass `--url <orchestrator-url>` or set ORCHESTRATOR_URL.',
			),
		)
		return
	}
	if (!isValidUrl(url)) {
		checks.push(
			fail(
				'orchestrator-health',
				'Orchestrator health',
				`Cannot check invalid URL: ${url}.`,
				'Pass a full URL such as `http://localhost:7778`.',
			),
		)
		return
	}
		...
		checks.push(
			res.ok
				...
				: fail(
						'orchestrator-health',
						'Orchestrator health',
						`${url}/api/health returned HTTP ${res.status}.`,
						'Check orchestrator logs and URL routing.',
					),
		)
	} catch (err) {
		checks.push(
			fail(
				'orchestrator-health',
				'Orchestrator health',
				`Could not reach ${url}/api/health: ${err instanceof Error ? err.message : String(err)}`,
				'Start the orchestrator or pass the reachable URL.',
			),
		)
	}
}
...
function runtimeInstallHint(runtime: string): string {
	switch (runtime) {
		...
		case 'codex':
			return 'Configure OpenAI auth for the spawn-agent Codex ACP runtime.'
		case 'opencode':
...
function printDoctor(checks: DoctorCheck[]): void {
	...
	console.log(table(checks.map((check) => [statusBadge(check.status), check.title, check.message])))
