...
describe('findCompanyRoot', () => {
	...
	it('finds company.yaml in the given directory', async () => {
		await writeFile(join(tempDir, 'company.yaml'), 'name: test')
		const root = await findCompanyRoot(tempDir)
		expect(root).toBe(tempDir)
	})

	it('finds company.yaml in a parent directory', async () => {
		await writeFile(join(tempDir, 'company.yaml'), 'name: test')
	...
	it('throws when company.yaml is not found', async () => {
		...
		try {
			await expect(findCompanyRoot(isolatedDir)).rejects.toThrow('Could not find company.yaml')
		} finally {
