	...
	beforeAll(async () => {
		...
		const script = buildClaudeScript([
			...
			},
			{
				...
				message: {
					content: [
						{
							type: 'text',
							text: 'Task completed successfully. I analyzed the codebase and made the requested changes.',
						},
					],
				},
			},
			{
				...
				subtype: 'success',
				result:
					'Task completed successfully. I analyzed the codebase and made the requested changes.',
				session_id: 'test-session-123',
...
`
	...
	test('prefers Claude result error over generic exit code', async () => {
		const errorBinaryPath = join(tmpDir, 'claude-result-error-exit-1')
		await writeFile(
			errorBinaryPath,
			`#!/bin/bash
echo '{"type":"system","subtype":"init","session_id":"err-session"}'
echo '{"type":"result","subtype":"error","error":"Claude usage limit reached"}'
exit 1
`,
		)
		await chmod(errorBinaryPath, 0o755)
		...
		await expect(adapter.start(context)).rejects.toThrow('Claude usage limit reached')

		const errorEvents = events.filter((e) => e.type === 'error')
		expect(errorEvents.some((e) => e.summary === 'Claude usage limit reached')).toBe(true)
	})
})
