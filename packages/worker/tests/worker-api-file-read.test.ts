/**
 * Tests for the /workspaces/:runId/read endpoint on the worker API.
 *
 * Covers:
...
import { type WorkerApiDeps, createWorkerApi } from '../src/api'
...
// ─── Workspace read endpoint tests ──────────────────────────────────────────────

describe('GET /workspaces/:runId/read', () => {
	let repoRoot: string
	...
	beforeAll(async () => {
		...
		Bun.spawnSync(['git', 'commit', '-m', 'add test files'], {
			cwd: wtPath,
			stdout: 'pipe',
			stderr: 'pipe',
		})

		const result = createTestApi(
			makeDeps({
				repoRoot,
	...
	test('read text file returns 200 with correct Content-Type and text inspection header', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=src/index.ts')
		...
		expect(res.headers.get('X-Workspace-Inspection-Type')).toBe('file')
		const body = await res.text()
		expect(body).toContain('export const hello')
	})

	test('read binary file returns 200 with correct Content-Type and binary inspection header', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=src/logo.png')
		expect(res.status).toBe(200)
		expect(res.headers.get('Content-Type')).toBe('image/png')
		expect(res.headers.get('X-Workspace-Inspection-Text')).toBe('false')
	...
	test('path traversal is blocked with 400', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=../../etc/passwd')
	...
	test('blocked path .git/config returns 403', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=.git/config')
	...
	test('blocked path node_modules/foo returns 403', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=node_modules/foo')
		expect(res.status).toBe(403)
		const data = (await res.json()) as any
		expect(data.error).toContain('blocked')
	})
	...
	test('blocked path .env.local returns 403', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=.env.local')
		expect(res.status).toBe(403)
		const data = (await res.json()) as any
		expect(data.error).toContain('blocked')
	})
	...
	test('nonexistent file returns 404', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=src/doesnotexist.ts')
		expect(res.status).toBe(404)
		const data = (await res.json()) as any
		expect(data.error).toContain('not found')
	})

	test('directory path returns 400 (not readable content)', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read?path=src/subdir')
		expect(res.status).toBe(400)
		const data = (await res.json()) as any
		expect(data.error).toContain('not readable content')
	})

	test('missing path query param returns 400', async () => {
		const res = await fetchApi(app, '/workspaces/run-file-test/read')
		expect(res.status).toBe(400)
	})

	test('nonexistent workspace returns 404', async () => {
		const res = await fetchApi(app, '/workspaces/run-does-not-exist/read?path=src/index.ts')
		expect(res.status).toBe(404)
		const data = (await res.json()) as any
		expect(data.error).toContain('not found')
	})
