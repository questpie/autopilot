...
import { type RuntimeConfig, resolveRuntime } from '../src/runtime-config'
...
describe('resolveRuntime', () => {
	...
	test('adapter exposes the RuntimeAdapter interface', async () => {
	...
	test('runtime config resolves to spawn-agent adapter', async () => {
		...
		expect(resolved.adapter.constructor.name).toBe('SpawnAgentAdapter')
		expect(typeof resolved.adapter.start).toBe('function')
	})
})
