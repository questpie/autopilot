...
function resolveClaudeErrorMessage({
	exitCode,
	stderr,
	runtimeError,
}: {
	...
	const trimmedRuntimeError = runtimeError?.trim() ?? ''

	if (trimmedRuntimeError && trimmedStderr && trimmedRuntimeError !== trimmedStderr) {
		return `${trimmedRuntimeError}\n\n${trimmedStderr}`
	}

	return trimmedRuntimeError || trimmedStderr || `Claude exited with code ${exitCode}`
}
...
export class ClaudeCodeAdapter implements RuntimeAdapter {
	...
	async start(context: RunContext): Promise<RuntimeResult | undefined> {
		...
		args.push('--dangerously-skip-permissions', '--max-turns', String(this.config.maxTurns ?? 50))
		...
		try {
			...
			const { sessionId, lastResult, tokens, runtimeError } = await this.streamJsonl(proc)
			...
			if (exitCode !== 0) {
				const errorMsg = resolveClaudeErrorMessage({ exitCode, stderr, runtimeError })
	...
	private async streamJsonl(proc: Subprocess): Promise<{
		...
		if (!stdout || typeof stdout === 'number')
			return { sessionId, lastResult, tokens, runtimeError }
		...
		await streamLines(stdout as ReadableStream<Uint8Array>, (line) => {
			try {
				...
				if (result.runtimeError) runtimeError = result.runtimeError
			} catch {
		...
		switch (event.type) {
			...
			case 'result': {
				if (event.subtype === 'error') {
					const runtimeError = event.error?.trim() || 'Claude Code result error'
					this.emit({ type: 'error', summary: runtimeError })
					return { runtimeError }
				}
				// subtype === 'success'
