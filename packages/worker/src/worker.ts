...
import { type ActionsMergedResult, executeActions, mergeOutputs } from './actions/webhook'
import { type WorkerApiConfig, type WorkerApiServer, startWorkerApi } from './api'
import { type StoredCredential, loadCredential, saveCredential } from './credentials'
...
import type { RunContext, RuntimeAdapter } from './runtimes/adapter'
...
export class AutopilotWorker {
	...
	async start(): Promise<void> {
		...
		if (this.workerId) {
			try {
				...
				console.debug(
					'[worker] pre-start deregister (best effort):',
		...
		this.pollTimer = setInterval(() => this.poll(), this.config.pollInterval ?? 5_000)
	...
	private async executeRun(run: ClaimedRun): Promise<void> {
		...
		await this.postEvent(run.id, {
			...
			metadata: ws
				? {
						...
						workspace_degraded: ws.degraded,
					}
				: undefined,
		})
		...
		try {
			adapter.onEvent((event) => {
				this.postEvent(run.id, event).catch((err) =>
					console.warn('[worker] failed to post event:', err),
				)
			})
			...
			const mergedOutputs = mergeOutputs(result?.outputs ?? {}, actionResult?.outputs ?? {})
	...
	private async runPostActions(
		...
		return executeActions({
			...
			emitEvent: (event) => {
				this.postEvent(run.id, event).catch((err) =>
					console.warn('[worker] failed to post action event:', err),
				)
			},
	...
	private async cleanupStaleWorktrees(): Promise<void> {
		...
		for (const entry of entries) {
			...
			try {
				const run = (await this.api(`/api/runs/${encodeURIComponent(entry)}`, {
					method: 'GET',
				})) as {
					id: string
					task_id?: string
