...
interface DriftSummary {
  ...
  ahead: number
  behind: number
  changed_files: { path: string; status: string; old_path?: string }[]
  dirty_count: number
}

interface WorkspaceDetail extends WorkspaceEntry {
...
function getChangedFiles(cwd: string, base: string): { path: string; status: string; old_path?: string }[] {
...
function getDiffStats(cwd: string, base: string): { files_changed: number; insertions: number; deletions: number } {
  ...
  for (const line of result.stdout.split('\n')) {
    ...
    if (match) {
      files++
      if (match[1] !== '-') insertions += parseInt(match[1]!, 10) || 0
      if (match[2] !== '-') deletions += parseInt(match[2]!, 10) || 0
    }
  }
...
export function createWorkerApi(deps: WorkerApiDeps, config?: WorkerApiConfig) {
  ...
  app.get('/workspaces/:runId', (c) => {
    ...
    const detail: WorkspaceDetail = {
      ...
      path: wtPath,
      branch,
      created: false, // Can't tell retroactively
      degraded: false,
  ...
  app.get('/workspaces/:runId/diff', (c) => {
    const workspace = deps.getWorkspace()
    if (!workspace) {
      return c.json({ error: 'no workspace manager configured' }, 404)
    }
  ...
  app.get('/workspaces/:runId/files', async (c) => {
    const workspace = deps.getWorkspace()
    if (!workspace) {
      return c.json({ error: 'no workspace manager configured' }, 404)
    }
...
function listWorkspaces(workspace: WorkspaceManager, activeRunId: string | null): WorkspaceEntry[] {
  const repoRoot = (workspace as any).repoRoot as string
