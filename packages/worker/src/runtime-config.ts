...
import { SpawnAgentAdapter } from './runtimes/spawn-agent'
...
function resolveBinaryPath(config: RuntimeConfig, binaryName: string): string {
	// spawn-agent's Codex support is provided by @zed-industries/codex-acp, not
	// by shelling out to a `codex` CLI binary.
	if (config.runtime === 'codex' && !config.binaryPath) {
		return binaryName
	}
	...
	switch (config.runtime) {
		...
		case 'codex': {
			if (config.maxTurns !== undefined) {
				console.warn(
					`[runtime] maxTurns=${config.maxTurns} configured for codex runtime, but Codex CLI has no max-turns equivalent. The setting will be ignored.`,
				)
			}
			if (config.binaryPath) {
				console.warn(
					'[runtime] binaryPath configured for codex runtime, ' +
						'but spawn-agent uses bundled @zed-industries/codex-acp. The setting validates availability only.',
				)
			}
			return new SpawnAgentAdapter({
				runtime: config.runtime,
		...
		case 'opencode': {
			if (config.maxTurns !== undefined) {
				console.warn(
					`[runtime] maxTurns=${config.maxTurns} configured for opencode runtime, but OpenCode CLI has no max-turns equivalent. The setting will be ignored.`,
				)
			}
			return new SpawnAgentAdapter({
				runtime: config.runtime,
