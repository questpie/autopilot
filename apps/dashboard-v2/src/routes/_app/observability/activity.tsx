...
export const Route = createFileRoute("/_app/observability/activity")({
  component: ObservabilityActivityTab,
...
function ObservabilityActivityTab() {
  ...
  const filteredEntries =
    actionFilter === "all"
      ? entries
      : entries.filter((e) => e.type === actionFilter)
          ...
          <div className="relative">
            ...
            <AnimatePresence mode="popLayout">
              {filteredEntries.map((entry) => (
                <div
                  key={`${entry.at}-${entry.agent}-${entry.type}`}
                  className="relative flex items-start gap-3 py-1"
                >
                  <div className="relative z-10 mt-3.5 ml-[19px] h-2 w-2 shrink-0 rounded-full border border-border bg-background" />
                  <div className="min-w-0 flex-1">
                    <ActivityItemRow entry={entry} showDate />
                  </div>
                </div>
              ))}
            </AnimatePresence>
