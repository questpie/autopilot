		...
		<div className="flex flex-col gap-8">
			...
			<div className="flex flex-col gap-2">
				...
				<p className="text-center text-sm text-muted-foreground">
					{useBackup ? t('auth.two_factor_backup_placeholder') : t('auth.two_factor_description')}
				</p>
			</div>
			{/* Error */}
			<AnimatePresence>
			...
			{failCount >= 3 && !useBackup && (
				<Alert variant="warning">
					<WarningCircleIcon className="size-4" />
					<AlertDescription>{t('auth.two_factor_failed_attempts')}</AlertDescription>
				</Alert>
			)}
			{useBackup ? (
				/* Backup code input */
				<form onSubmit={handleBackupSubmit} className="flex flex-col gap-4">
					<div className="flex flex-col gap-1.5">
						...
						<Input
							...
							value={backupCode}
							onChange={(e) =>
								dispatch({ type: 'SET_BACKUP_CODE', backupCode: e.currentTarget.value })
							}
				...
				<div className="flex flex-col gap-4">
					<div className="flex justify-between gap-2.5" onPaste={handleDigitPaste}>
						{digits.map((digit, i) => (
							<input
			...
			<div className="flex items-center gap-2">
				<Checkbox
					...
					onCheckedChange={(checked) =>
						dispatch({ type: 'SET_TRUST_DEVICE', trustDevice: checked === true })
					}
					id="trust-device"
				/>
			...
			<div className="text-center">
				...
				>
					{useBackup ? t('auth.two_factor_use_totp') : t('auth.two_factor_use_backup')}
				</button>
			</div>
		</div>
	)
