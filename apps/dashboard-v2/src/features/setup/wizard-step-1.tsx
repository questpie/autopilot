...
export function WizardStep1({ onComplete }: WizardStep1Props) {
	...
	const onSubmit = async (values: AccountValues) => {
		...
		if (result.error) {
			form.setError('root', {
				message: result.error.message ?? t('errors.account_creation_failed'),
			})
			return
		}
	...
	return (
		<div className="flex flex-col gap-6">
			<div>
				<h2 className="font-heading text-xl font-semibold">{t('setup.step_1_title')}</h2>
				<p className="mt-1 text-sm text-muted-foreground">{t('setup.step_1_description')}</p>
			</div>
