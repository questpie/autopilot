...
import { ArrowLeftIcon, CloudIcon, WarningCircleIcon } from '@phosphor-icons/react'
...
export function WizardStep3({ onComplete, onBack }: WizardStep3Props) {
	...
	const { completeStep, setProviderChoice } = useWizardState()
	...
	if (deploymentMode === 'cloud') {
		return (
			<div className="flex flex-col gap-6">
				<div>
					<h2 className="font-heading text-xl font-semibold">{t('setup.step_3_title')}</h2>
				...
				<div className="flex items-center gap-3 border border-primary/30 bg-primary/5 p-4">
					...
					<div>
						<p className="font-heading text-sm font-medium">{t('setup.step_3_cloud_managed')}</p>
						<p className="text-xs text-muted-foreground">{t('setup.step_3_cloud_managed_desc')}</p>
					</div>
				</div>
	...
	return (
		<div className="flex flex-col gap-6">
			<div>
				<h2 className="font-heading text-xl font-semibold">{t('setup.step_3_title')}</h2>
				<p className="mt-1 text-sm text-muted-foreground">{t('setup.step_3_description')}</p>
			</div>
