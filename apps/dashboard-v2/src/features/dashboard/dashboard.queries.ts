...
export const dashboardGroupsQuery = queryOptions({
  ...
  queryFn: async () => {
    ...
    if (!res.ok && res.status !== 404) throw new Error("Failed to fetch groups")
    if (res.status === 404) return { groups: [] }
    return res.json() as Promise<{ groups: Array<{ id: string; title: string; icon?: string; position: number }> }>
  },
})
