...
import { FileAttachButton, UploadPreviewBar } from './upload-controls'
...
export function MessageInput({
	...
	const handleSendOrSteer = useCallback(() => {
		...
		if (isStreaming && activeSessionId) {
			...
			fetch(`${API_BASE}/api/agent-sessions/${encodeURIComponent(activeSessionId)}/steer`, {
				method: 'POST',
	...
	return (
		<div className="border-t border-border">
			{/* Reply bar */}
			{replyingTo && <ReplyBar replyingTo={replyingTo} onCancel={clearReplyingTo} />}
			...
			{showPastePrompt && (
				<div className="border-t border-border bg-muted/30 px-3 py-2 text-xs">
					<div className="mb-1.5 flex items-center gap-2">
						<span className="flex-1 font-medium text-foreground/80">Large paste detected</span>
			...
			<div className="relative flex items-end gap-2 px-3 py-2">
				...
				<textarea
					...
					placeholder={
						isStreaming && activeSessionId ? 'Steer the conversation...' : t('chat.type_message')
					}
					rows={1}
					className={cn(
			...
			{effectiveQueuedMessage && <QueuedMessageIndicator message={effectiveQueuedMessage} />}
		</div>
	)
}
