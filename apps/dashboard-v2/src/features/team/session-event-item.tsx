 ...
 * - action: amber (send_message, pin_to_board, etc.)
 */
function getToolCategory(toolName?: string): "read" | "write" | "action" {
  if (!toolName) return "read"
  const name = toolName.toLowerCase()
  if (name.includes("write") || name.includes("create") || name.includes("update") || name.includes("delete")) {
    return "write"
  }
  if (name.includes("send") || name.includes("pin") || name.includes("approve") || name.includes("reject") || name.includes("deploy")) {
    return "action"
  }
  return "read"
}
...
function getToolIcon(toolName?: string) {
  ...
  if (name.includes("send_message")) return ChatCircleIcon
