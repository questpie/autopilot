...
const secretSchema = z.object({
  name: z.string().min(1, "Name is required").regex(/^[a-z0-9-]+$/, "Lowercase letters, numbers, and hyphens only"),
...
export function SecretAddDialog({ open, onOpenChange }: SecretAddDialogProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: SecretFormValues) => {
      const yaml = buildSecretYaml(values)
      const res = await api.api.files[":path{.+}"].$post({
        param: { path: `secrets/${values.name}.yaml` },
        json: { content: yaml },
      })
...
function buildSecretYaml(values: SecretFormValues): string {
  const lines = [
    `name: ${values.name}`,
    `type: ${values.type}`,
    `value: ${values.value}`,
