...
/** Onboarding feature reads (ADR 0022 / 0023). */
export function createOnboardingQueries(q: AppQueryOptions) {
	const keys = createQueryKeys(q);
	return {
		/**
		 * Onboarding-state query: same companies.visible read, derived shape, own
		 * cache entry. Casts bridge the workspace's duplicated @tanstack/react-query
		 * identity (see lib/data/session.ts); the builder's queryFn ignores its
		 * context argument at runtime.
		 */
			...
			return q.custom.query({
				...
				queryFn: async (): Promise<OnboardingState> => {
					try {
						...
						// A session with no membership is denied the companies read (the access
						// rule answers false -> 403). For onboarding that denial IS the empty
						// projection: no visible company yet. Every other failure stays a failure.
