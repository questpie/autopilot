...
function isEmptyRun(run: Run): boolean {
	return (
		!run.summary ||
		run.summary.trim() === '' ||
...
export function buildRunsTimeline(taskRuns: Run[]): TimelineEntry[] {
	return [...taskRuns]
		.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
		.map((run, index) => {
			let status: TimelineStatus = 'done'
			if (run.status === 'running' || run.status === 'claimed') {
				...
				status = 'failed'
			} else if (isEmptyRun(run) && run.status === 'completed') {
				status = 'empty'
			}

			const baseLabel = `Run ${index + 1}`
			const label = run.resumed_from_run_id ? `${baseLabel} (resume)` : baseLabel
