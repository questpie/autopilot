...
export function WorkflowTimeline({
	...
	const { openDraftChat, openSession } = useChatWorkspace()
	...
	function openRunChat(entry: TimelineEntry, sessionId?: string) {
		if (sessionId) {
	...
	return (
		<div className={cn('space-y-0', className)}>
			{entries.map((entry, i) => {
				...
				const artifacts = entry.run ? (artifactsByRunId[entry.run.id] ?? []) : []
							...
							{(sessionId || entry.run?.runtime_session_ref) && (
								<div className="mt-1 ml-4 flex flex-wrap gap-1.5">
									{sessionId ? (
										...
										<Button onClick={() => openRunChat(entry)} size="xs" variant="outline">
											<ChatCircle data-icon="inline-start" />
								...
								<div className="mt-1.5 ml-4 space-y-1.5">
									...
									{entry.run && (
										<div className="flex items-center gap-2">
											<span className="text-xs text-foreground-subtle">{entry.run.agent_id}</span>
											<span className="text-xs text-foreground-subtle tabular-nums">
												{entry.run.id.slice(0, 12)}&hellip;
											</span>
											{entry.run.model && (
												<span className="text-xs text-foreground-subtle">{entry.run.model}</span>
											)}
											{entry.run.worker_id && (
												<span className="text-xs text-foreground-subtle tabular-nums">
													worker:{entry.run.worker_id.slice(0, 8)}
												</span>
											)}
										</div>
									)}
