				...
				<SidebarMenu>
					{TASK_FILTERS.map((f) => (
						<SidebarMenuItem key={f}>
							<SidebarMenuButton
								render={<Link to="/tasks" search={f === 'all' ? {} : { filter: f }} />}
...
function FilesSidebar() {
	...
	const currentPath =
		search.view === 'file' && rawPath ? rawPath.split('/').slice(0, -1).join('/') : rawPath
							...
							return (
								<SidebarMenuItem key={entry.path}>
									...
									>
										<EntryIcon
											size={14}
											weight={entry.type === 'directory' ? 'fill' : 'regular'}
											className="shrink-0 text-muted-foreground"
										/>
		...
		<>
			<SidebarGroup>
				...
				<SidebarGroupContent>
					<SidebarMenu>
						<SidebarMenuItem>
							<SidebarMenuButton render={<Link to="/chat" />} className="font-mono text-xs">
						...
						<SidebarMenuItem>
							<SidebarMenuButton render={<Link to="/files" />} className="font-mono text-xs">
