...
import { workspaceInspectionContentUrl } from '@/api/workspace-inspection.api'
import { Button } from '@/components/ui/button'
import { InspectorHeader } from '@/components/ui/inspector-layout'
import { Spinner } from '@/components/ui/spinner'
import { surfaceCardVariants } from '@/components/ui/surface-card'
import { TiptapEditor } from '@/components/ui/tiptap-editor'
import { knowledgeSourceKeys, useKnowledgeRead } from '@/hooks/use-knowledge-source'
...
import { formatBytes } from '../lib/knowledge-locations'
...
export function ResourceView({ path, runId, projectId, onBack, onOpenFile }: ResourceViewProps) {
	const contentUrl = runId
		? workspaceInspectionContentUrl(runId, path)
		: knowledgeContentUrl(path, { projectId })
	const { data, isLoading, error } = useKnowledgeRead(runId, path, projectId)
	...
	const [markdownSource, setMarkdownSource] = useState<{ sourceUrl: string; body: string } | null>(
		null,
	)
	...
	useEffect(() => {
		...
		setMarkdownSource({ sourceUrl: contentUrl, body: nextContent })
	...
	const isEditable =
		...
		(viewer.type === 'code' ||
			...
			viewer.type === 'markdown' ||
			viewer.type === 'openapi')
	const headerTitle = runId ? `Project run / ${path}` : `Knowledge / ${path}`
			...
			{!isLoading &&
				...
				(() => {
					...
					if (viewer.type === 'markdown') {
						...
						const editorContent =
							markdownSource?.sourceUrl === contentUrl && markdownSource.body === savedMarkdownBody
								? markdownDraft
								: savedMarkdownBody
							...
							<div ref={markdownScrollRef} className="relative h-full overflow-auto">
								...
								<TiptapEditor
									...
									onChange={(md) => {
										...
										setMarkdownDraft(md)
										setMarkdownSource({ sourceUrl: contentUrl, body: savedMarkdownBody })
