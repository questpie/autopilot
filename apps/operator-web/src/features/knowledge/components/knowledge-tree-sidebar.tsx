...
import { useKnowledgeList } from '@/hooks/use-knowledge-source'
import { fileIcon } from '@/lib/file-icons'
import { cn } from '@/lib/utils'
import { CaretDown, CaretRight } from '@phosphor-icons/react'
import { useEffect, useState } from 'react'
import { buildPathSegments, getBaseName } from '../lib/knowledge-locations'
...
interface TreeBranchProps extends KnowledgeTreeSidebarProps {
	path: string | null
	level: number
	expanded: Set<string>
...
function TreeEntry({
	...
	expanded,
	onToggle,
}: Omit<TreeBranchProps, 'path'> & { entry: ResourceListEntry }) {
...
function TreeBranch({
	...
	const { data, isLoading, isError } = useKnowledgeList(runId, path, projectId)
...
export function KnowledgeTreeSidebar({
	...
	const rootLabel = runId ? `Project run ${runId.slice(0, 8)}` : 'Knowledge'
	const currentName = selectedPath ? getBaseName(selectedPath) : 'All resources'
