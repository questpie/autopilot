...
import { useKnowledgeList, useKnowledgeRead, useKnowledgeStat } from '@/hooks/use-knowledge-source'
...
import { type SavedKnowledgeLocation, formatBytes, getBaseName } from '../lib/knowledge-locations'
import { ResourcePreviewSurface } from './resource-preview-surface'

interface KnowledgeSelectionInspectorProps {
...
export function KnowledgeSelectionInspector({
	currentPath,
	selectedPath,
	runId,
	projectId,
	...
	onOpenSelected,
	onTogglePinned,
}: KnowledgeSelectionInspectorProps) {
	const inspectPath = selectedPath ?? currentPath
	const inspectStat = useKnowledgeStat(runId, inspectPath, projectId)
	const inspectList = useKnowledgeList(
		runId,
		inspectStat.data?.type === 'directory' ? inspectPath : null,
		projectId,
	)
	const inspectRead = useKnowledgeRead(
		runId,
	...
	const contentUrl = inspectPath
		? runId
			? workspaceInspectionContentUrl(runId, inspectPath)
