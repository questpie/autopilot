import type { WorkspaceInspectionReadResult } from '@/api/workspace-inspection.api'
...
const OpenApiReferencePreview = lazy(() =>
	import('./openapi-reference-preview').then((module) => ({
		default: module.OpenApiReferencePreview,
	})),
)

type ResourcePreviewVariant = 'full' | 'inspector' | 'card'

interface ResourcePreviewSurfaceProps {
	path: string
	contentUrl: string
	viewerType: ViewerType
	data?: WorkspaceInspectionReadResult | null
	variant: ResourcePreviewVariant
			...
			<Suspense
				fallback={
					<div className="flex h-full items-center justify-center gap-2 text-muted-foreground">
						<Spinner size="sm" />
						<span className="text-sm">Loading OpenAPI renderer…</span>
					</div>
				}
			>
				<OpenApiReferencePreview path={path} content={data.content} variant={variant} />
			</Suspense>
		)
	}
