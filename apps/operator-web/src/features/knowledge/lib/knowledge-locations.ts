export interface SavedKnowledgeLocation {
	path: string | null
	runId: string | null
	type: 'file' | 'directory'
	label: string
...
export function getBaseName(path: string | null, fallback = 'Knowledge'): string {
...
export function locationKey(
	location: Pick<SavedKnowledgeLocation, 'path' | 'runId' | 'type'>,
): string {
	return `${location.runId ?? 'company'}:${location.type}:${location.path ?? '.'}`
}

export function normalizeSavedKnowledgeLocation(value: unknown): SavedKnowledgeLocation | null {
	if (typeof value !== 'object' || value === null) return null
	const candidate = value as Record<string, unknown>
	const path =
		typeof candidate.path === 'string' ? candidate.path : candidate.path === null ? null : null
	const runId =
		typeof candidate.runId === 'string' ? candidate.runId : candidate.runId === null ? null : null
	...
	const viewedAt =
		typeof candidate.viewedAt === 'string' ? candidate.viewedAt : new Date().toISOString()
	if (!type) return null
