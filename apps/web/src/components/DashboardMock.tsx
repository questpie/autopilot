		...
		<div className="bg-card border border-border overflow-hidden">
			...
			<div className="p-3 sm:p-4 flex flex-col gap-2.5">
				...
				{items.map((item) => (
					<div
						key={item.title}
						className="flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-3 px-3 py-2.5 bg-surface border border-border"
					>
				...
				<div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono">
					{agents.map((a) => (
						<div
							key={a.n}
							className="px-2.5 py-2 bg-surface border border-border"
						>
							<div className="text-fg">
								{a.s} {a.n}
							</div>
							<div className="text-dim text-[10px] mt-0.5 truncate">{a.d}</div>
						</div>
					))}
				</div>
			</div>
