...
`codex` and `opencode` are already declared in the type system and binary mapping. The adapters are not yet implemented, but the worker routing, capability advertisement, and run targeting already support multiple runtimes per worker.
