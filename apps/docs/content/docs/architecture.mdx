...
- **Config registry** — live runtime configuration for agents, workflows, teams, providers, schedules, renderers, and MCP servers.
- **`.autopilot/`** — import/export, seed, pack, local fixture, and migration format for config; not the live source of truth after import.

Every client (CLI, MCP, bound conversations, and Operator Web) operates over the same primitives. The orchestrator is the system of record. Workers execute. The config registry and Knowledge API define shared operational truth.
