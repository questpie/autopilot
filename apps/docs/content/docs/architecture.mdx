...
│       │
│       └── @questpie/autopilot-dashboard  (apps/dashboard)
│            React UI → calls orchestrator via REST API + SSE
```

## Provider Abstraction

The agent spawner uses a provider abstraction. Three providers are available: **Claude Agent SDK** (primary), **Anthropic SDK** (raw API with agentic loop), and **Codex SDK** (OpenAI). The Claude Agent SDK provider gives agents built-in infrastructure -- file I/O, shell access, sub-agents, MCP support, and lifecycle hooks. The Anthropic provider uses the raw `@anthropic-ai/sdk` with tool use. The Codex provider offers an OpenAI GPT runtime. Configure per-agent or company-wide in `company.yaml`.
...
Works with an Anthropic API key or a Claude Max subscription. Set `ANTHROPIC_API_KEY` for Claude models. The Anthropic provider uses the raw SDK with an agentic tool-use loop. Codex SDK is available for GPT models (gpt-4o, gpt-4.1, o3, o4-mini) via `OPENAI_API_KEY`.
...
| session/ | `SessionStreamManager` | In-memory session stream management. Powers `autopilot attach` for real-time observation. |
| api/ | `ApiServer` | REST API + SSE event stream on port 7778. Dashboard and CLI consumer endpoint. |
...
| notifier/ | `Notifier` | Logs notifications to stdout and activity feed. Telegram transport built-in, Slack/email planned. |
