---
title: Architecture
description: Single Bun process. One SQLite file. Zero external infrastructure. Four-layer stack, orchestrator internals, and filesystem-native state.
---
...
+-----------------------------------------------------------+
|  LAYER 3 -- AGENTS                                         |
|  Claude Agent SDK sessions with 13 primitives + MCP        |
...
The Claude Agent SDK provider gives agents built-in infrastructure -- file I/O, shell access, sub-agents, MCP support, and lifecycle hooks. Role-based filesystem scoping and deny patterns are enforced on top.
...
| context/ | Builds role-scoped system prompts from 4 layers: identity, company state, memory, and task context. |
...
Watcher: detects new tasks
  |
  v
Sam (strategist): writes spec -> update_task(status: done)
  v
Workflow Engine: scope -> plan transition -> activates Alex's task
  v
Alex (planner): writes plan -> update_task(status: done)
  v
Max (developer): implements, creates PR -> update_task(status: done)
  v
