...
.data/
  autopilot.db                # SQLite database (FTS5 search, embeddings, auth, sessions)
```
...
For production, run the company in an isolated Docker container. The container mounts the company filesystem and exposes webhook and API ports:
...
services:
  autopilot:
    ...
    ports:
      ...
      - "7778:7778"    # REST API + SSE event stream + dashboard
    environment:
