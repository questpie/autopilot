...
|----------|----------|---------|-------------|
| `ANTHROPIC_API_KEY` | No | — | Claude API key for agent sessions (not required if using a Claude Max/Pro subscription) |
...
Optional if using a Claude Max/Pro subscription. Otherwise, all agent sessions use this key to call the Claude API. Get one at [console.anthropic.com](https://console.anthropic.com).
