...
| `CORS_ORIGIN` | No | — | Allowed CORS origin for the API. Set to your domain URL when behind a reverse proxy (e.g., `https://autopilot.yourdomain.com`). Falls back to `company.yaml` `auth.cors_origin`, then `http://localhost:3001`. |

### ANTHROPIC_API_KEY

Alternative to subscription login. If you prefer API keys over `autopilot provider login claude`, set this variable. Get one at [console.anthropic.com](https://console.anthropic.com).
