...
Alternative to subscription login. If you prefer API keys over `autopilot provider set openrouter --api-key sk-or-...`, set this variable. Get one at [openrouter.ai/keys](https://openrouter.ai/keys).
...
```yaml title="company.yaml"
settings:
  agent_provider: "tanstack-ai"  # Primary: Claude Agent SDK
  agent_model: "claude-sonnet-4-6"    # Model for agent sessions
```

Supported providers:
- `tanstack-ai` — Anthropic Claude Agent SDK (primary, recommended)
- `tanstack-ai` — OpenAI Codex SDK (alternative)
